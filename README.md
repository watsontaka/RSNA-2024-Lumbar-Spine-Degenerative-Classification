...
そこで、該当する画像をニューラルネットワークで以下の枚数抽出することにしました。
確率が高いものから順に抽出しました。
...
ただ、x軸の平均を計算して平均以下の画像は右側・平均以上の画像は左側と区分分けしました。
...
**目的変数の作成**
csvファイルで症状が確認できる断面画像のデータがあるので、それをもとに目的変数を作成しました。
one-hot表現(該当する場合は1,それ以外は0)で
|ファイル名|該当しているか|
|:--:|:--:|
|1.dcm|なし|
|2.dcm|なし|
